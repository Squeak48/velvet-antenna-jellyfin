(function () {
    ...
    function tryPendingPlay() {
        ...
        if (button) {
            sessionRemove(PENDING_PLAY_KEY);
            clearPlayTimer();
            console.log('[Velvet Antenna v0.17.2] using Jellyfin local .btnPlay for', pending.id);
            button.click();
            return true;
        }
        ...
        if (playAttempts < MAX_PLAY_ATTEMPTS) {
            ...
            console.warn('[Velvet Antenna v0.17.2] no genuine local .btnPlay found for', pending.id);
            sessionRemove(PENDING_PLAY_KEY);
        }
        return false;
    }
    ...
    function start() {
        ...
        // v0.17.2 owns only Velvet Antenna action routing.
