(function () {
  ...
  const CHECKS = [0, 40, 100, 220, 450, 800, 1400, 2400, 3800];
  ...
  function clearIfReady() {
    if (!isHome() || document.getElementById('va224-home')) clear();
  }
  ...
  function boot() {
    ...
    CHECKS.forEach(delay => setTimeout(clearIfReady, delay));
  }

  if (document.body) boot();
  else document.addEventListener('DOMContentLoaded', boot, { once: true });
  addEventListener('hashchange', () => {
    if (isHome()) boot(); else clear();
  });
  addEventListener('va224:route', clearIfReady);

  window.VA226Bootstrap = { clear, boot };
